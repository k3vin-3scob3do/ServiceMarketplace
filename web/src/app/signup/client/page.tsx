...
export default function SignupClient() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await api("/auth/register/client", { method: "POST", body: JSON.stringify(form) });
      r.push("/#login");
    } catch (e: any) {
      setErr(e.message);
  ...
  return (
    <main className="mx-auto max-w-lg p-6">
      <h1 className="text-2xl font-semibold mb-4">Crear cuenta (Cliente)</h1>
      <form onSubmit={submit} className="border rounded-xl p-6 space-y-3">
        ...
        <input className="w-full border rounded px-3 py-2" placeholder="Teléfono (opcional)"
