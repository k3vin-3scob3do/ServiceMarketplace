...
export default function Navbar() {
  const { user: currentUser, logout, isReady } = useUser();
  ...
  return (
    <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b shadow-sm">
      <nav className="mx-auto max-w-6xl px-4 h-14 flex items-center justify-between">
        ...
        {!isReady ? (
          <div className="flex gap-6 text-gray-700 font-medium opacity-0 pointer-events-none">
        ...
        {!isReady ? (
          <div className="opacity-0 pointer-events-none">
            <button className="px-4 py-1.5 rounded border">...</button>
          </div>
        ) : !currentUser ? (
